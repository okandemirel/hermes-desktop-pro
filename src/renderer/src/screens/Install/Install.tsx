...
function TerminalLog({ lines, maxLines = 12 }: { lines: string[]; maxLines?: number }) {
  ...
  return (
    <div
      ref={ref}
      className="font-mono text-xs leading-relaxed rounded-lg p-4 overflow-y-auto"
      style={{ background: "#0D0D0D", border: "1px solid rgba(255,255,255,0.05)", height: "220px" }}
    >
      {visible.length === 0 && (
        <span className="text-white/25">Waiting for installation to begin...</span>
      )}
      {visible.map((line, i) => {
        ...
        return <div key={i} className="text-white/55">{text}</div>;
      })}
      {/* Blinking cursor when running */}
      {lines.length > 0 && (
        <span className="inline-block w-2 h-4 ml-0.5 bg-[#0A84FF] animate-pulse align-middle" />
      )}
    </div>
  );
}
      ...
      <div className="flex flex-col items-center">
        ...
        >
          {status === "done" ? (
            ...
            <LoaderCircle size={14} className="text-[#0A84FF] animate-spin" />
          ) : status === "error" ? (
        ...
        {!isLast && (
          <div
            className={`w-0.5 h-6 mt-1 transition-colors duration-300 ${
              status === "done" ? "bg-green-500/30" : "bg-white/5"
            }`}
          />
        )}
      </div>
...
export default function InstallView() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden" style={{ background: "#0D0D0D" }}>
      {/* ── Header ── */}
      <div className="flex-shrink-0 px-6 py-6 border-b border-white/5" style={{ background: "#1A1A1A" }}>
        <div className="flex items-center gap-3 mb-1">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(10,132,255,0.1)" }}>
            <Download size={18} className="text-[#0A84FF]" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-white/95">Install Hermes</h1>
        ...
        {(phase === "installing" || phase === "done" || phase === "error") && (
          <div className="mt-4">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] font-mono text-white/60">{progressPercent}%</span>
            </div>
            <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: "#242424" }}>
              <div
                className={`h-full rounded-full transition-all duration-500 ${
                  phase === "error" ? "bg-red-500" : "bg-[#0A84FF]"
                }`}
        ...
        <div className="max-w-2xl mx-auto p-6">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] font-medium text-white/30 uppercase tracking-wide">Install Log</span>
            </div>
          ...
          <div className="flex items-center gap-3">
            {phase === "idle" && (
              ...
              >
                <Download size={15} /> Install Hermes
              </button>
            )}
            ...
            {phase === "installing" && (
              <button
                disabled
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium opacity-60 cursor-not-allowed"
                style={{ background: "#242424", color: "#fff" }}
              >
                <LoaderCircle size={15} className="animate-spin" /> Installing...
              </button>
            )}
            ...
            {phase === "error" && (
              <>
                <button
                  ...
                  style={{ background: "#0A84FF", color: "#fff" }}
                >
                  <RotateCcw size={15} /> Retry Installation
                </button>
                <button
            ...
            {phase === "done" && (
              <button
                ...
                style={{ background: "#0A84FF", color: "#fff" }}
              >
                <ArrowRight size={15} /> Continue to Setup
              </button>
            )}
          </div>
        </div>
